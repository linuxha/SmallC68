...
extern int level1(int is[]);
extern int level2(int is[]);
extern int level3(int is[]);
extern int level4(int is[]);
extern int level5(int is[]);
extern int level6(int is[]);
extern int level7(int is[]);
extern int level8(int is[]);
extern int level9(int is[]);
extern int level10(int is[]);
extern int level11(int is[]);
extern int level12(int is[]);
extern int level13(int is[]);
...
/*
** true if is2's operand should be iDoubled
*/
iDouble(oper, is1, is2) int oper, is1[], is2[]; {
...
down2(oper, oper2, level, is, is2)
    ...
    if(is[TC]) {                    /* consant op unknown */
        ...
        gen(GETw2n, is[CV] << iDouble(oper, is2, is));
    }
        ...
        if(is2[TC]) {                 /* variable op constant */
            ...
            if(oper == ADD12) {         /* commutative */
                gen(GETw2n, is2[CV] << iDouble(oper, is, is2));
            }
            else {                      /* non-commutative */
                gen(MOVE21, 0);
                gen(GETw1n, is2[CV] << iDouble(oper, is, is2));
            }
        }
        else {                        /* variable op variable */
            gen(POP2, 0);
            if(iDouble(oper, is, is2)) gen(DBL1, 0);
            if(iDouble(oper, is2, is)) gen(DBL2, 0);
        }
    }
    if(oper) {
...
*/
calc2(unsigned int left, unsigned int right, int oper) {
    switch(oper) {

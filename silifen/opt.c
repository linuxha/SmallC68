...
main(int argc, char *argv) {
    ...
    if (argc < 2 ) { /* not enough arguments */
        perror("Optimiser R1.0 28/5/85 useage: opt infile [outfile]");
        exit(1);
    } else {
    ...
    while( fgets(line,LINELEN,input) ) { /* read till end of file */
        if ( strncmp( line, match6, LENMAT6 ) == 0 ) { /* matched an INCS */
            strcpy( save, line);
            fgets( line, LINELEN, input );
            while( strncmp( line, match6, LENMAT6 ) == 0 ) { /* keep accumulating INCS n's onto save */
        ...
        if( strncmp( line, match1, LENMAT1 ) == 0 ) {       /* it matched a getloc */
            strcpy( save, line );
            fgets(line,LINELEN,input);
            if ( strncmp( line, match2, LENMAT2 ) == 0 ) { /* it matched getloc, getwi */
                /* can junk line so */
                fgets(line,LINELEN,input);
                if ( strncmp( line, match2, LENMAT2 ) == 0 ) { /* matched getloc,getwi,getwi */
                    /* edit getloc -> getwsi */
                    save[4] = 'W';
                    save[5] = 'S';
                    save[6] = 'I';
                    fputs(save,output);
                    continue;
                } else if ( strncmp( line, match3, LENMAT3 ) == 0 ) { /* matched getloc,getwi,getbi */
                    /* edit getloc -> getbsi */
                    save[4] = 'B';
                    save[5] = 'S';
                    save[6] = 'I';
            ...
            } else if ( strncmp( line, match3, LENMAT3 ) == 0 ) { /* it matched getloc, getbi */
                /* edit saved line */
        ...
        } else if ( strncmp( line, match4, LENMAT4 ) == 0 ) { /* it matched a getw */
            strcpy( save, line );
            fgets(line,LINELEN,input);
            if ( strncmp( line, match2, LENMAT2 ) == 0 ) { /* it matched getw, getwi */
                /* edit getw -> gwwi */
                save[2] = 'W';
                save[3] = 'W';
                save[4] = 'I';
            ...
            if ( strncmp( line, match3, LENMAT3 ) == 0 ) { /* matched getw, getbi */
                /* edit getw -> gwbi */
                save[2] = 'W';
                save[3] = 'B';
                save[4] = 'I';
        ...
        if ( strncmp( line, matchcc, LENMATCC ) == 0 ) {
            while( strncmp( line, matchcc, LENMATCC ) == 0 ) { /* strip off multiple CCEQ's */
                fputs( line, output );
                fgets( line, LINELEN, input );
            }
            if ( strncmp( line, matchtst, LENMATTST ) == 0 ) { /* matched TEST */
